...
use rzip_rescomply_core::CompanyProfile;
use serde_json;
use std::fs;
use std::path::PathBuf;

use rzip_rescomply_core::{
    CheckResult, CompanyProfile, CounterpartyCheck, CounterpartyType, IndustryCode, Jurisdiction,
    RiskLevel, hash_identifier, hash_name,
};
use rzip_rescomply_crypto::{ComplianceProof, ComplianceProver};
...
enum ProofType {
    NoOperations,     // Нет операций с юрисдикцией
    AllScreened,      // Все контрагенты проверены
    WithinLimits,     // Соблюдение лимитов транзакций
    FullCompliance,   // Полный compliance report
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Prove {
            ...
            // Создаём тестовые транзакции (в реальности они будут из БД)
            ...
            let proof = match proof_type {
                ...
                ProofType::AllScreened => {
                    prover.prove_counterparty_screening(&company, sanctions_lists, period_start)?
                }
                ...
                ProofType::FullCompliance => {
                    let auditor_signature = auditor.map(|_| vec![0u8; 64]); // Заглушка
...
fn create_test_transactions(company: &CompanyProfile) -> Vec<zk_rescomply_core::Transaction> {
    // Создаём тестовые транзакции
    // В реальной системе они будут загружаться из БД

    vec![
        zk_rescomply_core::Transaction {
            id: "TEST_TX_001".to_string(),
            timestamp: Utc::now() - Duration::days(10),
            amount: 1_000_000_00, // 1,000,000 в копейках
            ...
            category: zk_rescomply_core::TransactionCategory::ServicesPayment,
...
fn print_proof_details(
    ...
    if let Some(company_file) = company_file {
        if let Ok(company_data) = fs::read_to_string(company_file) {
            if let Ok(company_profile) = serde_json::from_str(&company_data) {
                println!();
